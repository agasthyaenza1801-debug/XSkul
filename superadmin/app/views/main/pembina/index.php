<div class="flex items-center justify-between mb-8">
    <div>
        <h3 class="text-2xl font-black text-midnight">Daftar Pembina</h3>
        <p class="text-slate-400 font-medium">Kelola data seluruh pembina ekskul</p>
    </div>
    <button onclick="openModal('modalCreate')" class="px-6 py-3 bg-admin text-white font-bold rounded-2xl shadow-lg shadow-admin/20 hover:bg-adminDark transition-all flex items-center gap-2">
        <i class="bi bi-plus-lg"></i> Tambah Pembina
    </button>
</div>
        ...
        <table class="w-full text-left">
            <thead>
                <tr class="bg-slate-50/50">
                    <th class="px-8 py-5 text-[10px] font-black text-slate-400 uppercase tracking-widest">NIP / Nama</th>
                    <th class="px-8 py-5 text-[10px] font-black text-slate-400 uppercase tracking-widest">No. HP</th>
            ...
            <tbody class="divide-y divide-slate-50">
                ...
                <tr class="hover:bg-slate-50/50 transition-colors">
                    <td class="px-8 py-5">
                        <div class="font-bold text-midnight"><?= htmlspecialchars($p['nama']) ?></div>
                        <div class="text-xs text-slate-400 font-medium"><?= htmlspecialchars($p['nip']) ?></div>
                    </td>
                    <td class="px-8 py-5 text-sm font-semibold text-slate-600"><?= htmlspecialchars($p['no_hp'] ?? '-') ?></td>
                    <td class="px-8 py-5">
                        <span class="px-3 py-1 <?= $p['is_active'] ? 'bg-green-100 text-green-600' : 'bg-slate-100 text-slate-400' ?> text-[10px] font-black rounded-lg uppercase">
                            <?= $p['is_active'] ? 'Aktif' : 'Nonaktif' ?>
                        </span>
                    </td>
                    <td class="px-8 py-5">
                        <div class="flex justify-center gap-2">
                            <button onclick="openModalEdit(<?= htmlspecialchars(json_encode($p)) ?>)" class="p-2.5 bg-slate-100 text-slate-400 hover:text-admin rounded-xl transition-all">
                                <i class="bi bi-pencil"></i>
                            </button>
                            <button onclick="openModalDelete(<?= $p['id'] ?>, '<?= htmlspecialchars($p['nama']) ?>')" class="p-2.5 bg-slate-100 text-slate-400 hover:text-red-500 rounded-xl transition-all">
...
<div id="modalCreate" class="fixed inset-0 z-50 hidden items-center justify-center bg-midnight/50 backdrop-blur-sm">
    <div class="bg-white rounded-3xl shadow-xl w-full max-w-md mx-4 flex flex-col max-h-[90vh]">
        <div class="px-8 pt-8 pb-2"><h4 class="text-lg font-extrabold text-midnight">Tambah Pembina</h4></div>
        <form action="<?= APP_URL ?>/pembina/create" method="POST" class="flex flex-col flex-1 min-h-0" autocomplete="off">
            <div class="overflow-y-auto flex-1 px-8 pt-2 pb-4 space-y-4">
                <div>
                    ...
                    <input type="text" name="nip" required class="w-full mt-1.5 px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:border-admin focus:ring-4 focus:ring-admin/10 outline-none font-semibold text-sm">
                </div>
                <div>
                ...
                <div>
                    <label class="text-xs font-bold text-slate-400 uppercase tracking-widest">No. HP</label>
                    <input type="text" name="no_hp" class="w-full mt-1.5 px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:border-admin focus:ring-4 focus:ring-admin/10 outline-none font-semibold text-sm">
                </div>
                <div>
...
<div id="modalEdit" class="fixed inset-0 z-50 hidden items-center justify-center bg-midnight/50 backdrop-blur-sm">
    <div class="bg-white rounded-3xl shadow-xl w-full max-w-md mx-4 flex flex-col max-h-[90vh]">
        <div class="px-8 pt-8 pb-2"><h4 class="text-lg font-extrabold text-midnight">Edit Pembina</h4></div>
        <form id="formEdit" action="" method="POST" class="flex flex-col flex-1 min-h-0" autocomplete="off">
            <div class="overflow-y-auto flex-1 px-8 pt-2 pb-4 space-y-4">
                <div>
                    <label class="text-xs font-bold text-slate-400 uppercase tracking-widest">NIP</label>
                    <input type="text" name="nip" id="editNip" required class="w-full mt-1.5 px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:border-admin focus:ring-4 focus:ring-admin/10 outline-none font-semibold text-sm">
                </div>
                <div>
                    <label class="text-xs font-bold text-slate-400 uppercase tracking-widest">Nama</label>
                ...
                <div>
                    <label class="text-xs font-bold text-slate-400 uppercase tracking-widest">No. HP</label>
                    <input type="text" name="no_hp" id="editNoHp" class="w-full mt-1.5 px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:border-admin focus:ring-4 focus:ring-admin/10 outline-none font-semibold text-sm">
                </div>
            </div>
            <div class="px-8 pb-8 pt-4 border-t border-slate-100 flex gap-3">
...
<div id="modalDelete" class="fixed inset-0 z-50 hidden items-center justify-center bg-midnight/50 backdrop-blur-sm">
    <div class="bg-white rounded-3xl shadow-xl w-full max-w-sm mx-4 p-8 text-center">
        ...
        <h4 class="text-lg font-extrabold text-midnight mb-1">Hapus Pembina?</h4>
...
<script>
    ...
    function openModalEdit(data) {
        ...
        document.getElementById('formEdit').action = '<?= APP_URL ?>/pembina/edit/' + data.id;
        openModal('modalEdit');
    }
    function openModalDelete(id, nama) {
        document.getElementById('deleteNama').textContent = nama;
        document.getElementById('formDelete').action      = '<?= APP_URL ?>/pembina/delete/' + id;
